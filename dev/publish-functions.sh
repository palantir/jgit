...
set_version() {
  version=$(get_version)
  mvn -f pom.xml -Dtycho.mode=maven tycho-versions:set-version -DnewVersion="$version"
  mvn -f org.eclipse.jgit.packaging/pom.xml -Dtycho.mode=maven tycho-versions:set-version -DnewVersion="$version"
}
